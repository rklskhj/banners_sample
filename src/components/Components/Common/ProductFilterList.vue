<template>
<div v-if="labelList && labelList.length > 0" style="padding: 8px 16px; background-color: #f3f3f3"
    class="full-width overflow-hidden" 
    v-dragscroll.x="true" @dragscrollmove.prevent.self="onDrag"
    :class="{'dragscroll': isIE}" ref="dragscroll">
    <div class="grab-bing flex-align full-width" style="gap: 0 12px">
      <!--<div class="unselect size-12" @click="clearFilter"-->
      <!--  style="padding: 4px 12px; border-radius: 4px; border: 1px solid #e8e8e8"-->
      <!--  :style="[brandBgColor.white, brandColor.main]">초기화</div>-->
      <div v-for="(label, idx) in labelList" :key="'filter-'+idx" class="flex-align">
        <div class="p-bg-primary" :style="btnStyle">
          <div class="size-12 position-relative flex-between p-white">
              {{ label.label }}
              <i @click="removeFilterItem(label.id, label.parent_id)" class="material-icons unselect flex-center p-white"
                style="width: 14px; height: 14px; margin-left: 2px; margin-bottom: 2px; font-size: 14px">close</i>
          </div>
        </div>
      </div>
    </div>
  ...
  methods: {
    removeFilterItem(id, parent_id) {
      this.filterData.forEach(item => {
        let labelList = this.cloneItem(this.labelList)
        if (parent_id === item.id) {
          if (item.type === 'choice_checkbox') {
            ...
            //   let index = this.labelList.findIndex(label => label.id === id && label.parent_id === item.id);
            //   this.labelList.splice(index, 1)
            // }
            this.labelList.forEach((label, idx) => {
              if (label.parent_id === item.id) {
                item.value = item.value.filter(i => i !== id)
                item.set[0].value = item.set[0].value.filter(i => i !== id)
                let selected = [];
                item.value.forEach(val => {
                  let opt_idx = item.options.findIndex(option => option.id === val);
                  if (opt_idx > -1) {
                    selected.push(item.options[opt_idx].label)
                  }
                })
                item.selectedValue = selected.toString();
                // labelList.splice(idx, 1)
                labelList = labelList.filter(i => i.id !== id)
              }
            })
            ...
            this.labelList.forEach((label, idx) => {
              if (label.id === id && label.label === item.value) {
                item.value = ''
                item.set[0].value = ''
                item.selectedValue = ''
                labelList.splice(idx, 1);
      ...
      this.$axios.get(`/public/service/set/filter/${this.filterId}`).then(res => {
        if (res.status === 200) {
          res.data.filter.forEach((filter, idx) => {
            ...
            if (filter.type === 'text') {
              store = this.$store.getters.filtered.filter(item=>{
                return item.keyword === filter.keyword && item.elType === filter.elType && item.label === filter.label;
              });
            }

            if (type === 'c-input-checkboxGroup' || filter.type === 'choice_checkbox') {
              store = this.$store.getters.filtered.filter(item => {
                return item.keyword === filter.keyword && item.param_id === filter.param_id && item.label === filter.label
              })
            }
            ...
            if(type === 'c-input-text') {
              if (store!== undefined) {
                ...
                filter.set[0].value = store.value;
              }
            }
            
            // 색상
            else if (filter.type === 'c-input-color') {
              if (store!==undefined) {
                filter.value = store.value;
            ...
            // 체크박스 그룹
            else if(type === 'c-input-checkboxGroup' || filter.type === 'choice_checkbox') {
              // 버튼 모양 변경 - 'full'
          ...
          filterData.forEach(item => {
            if (item.type === 'text' && item.value) {
              if (item.value === '') {
                ...
                labelArray.push({label: item.value, id: item.id, parent_id: item.id})
              }
            }
            ...
            if (item.type === 'choice_checkbox') {
              ...
              if (item.set[0].value === '' && item.set[1].value === '') {
                ...
                labelArray.push({label: `${item.set[0].value} ~ ${item.set[1].value}`, id: item.id, parent_id: item.id})
              }
            }
          })
