...
<div v-if="labelList && labelList.length !== 0" v-dragscroll.x="true" @dragscrollmove.prevent.self="onDrag"
  ...
  <div class="grab-bing flex-align" v-click-outside="close">
    <!--<div class="unselect size-12" @click="clearFilter"-->
    <!--  style="padding: 6px 12px; border-radius: 4px; background-color: #dddddd"-->
    <!--  :style="brandColor.main">초기화</div>-->
    <div v-for="(label, idx) in labelList" class="flex-align p-bg-primary p-white" :key="'label-'+idx" :style="btnStyle">
      ...
      <i @click="removeFilterItem(label.id, label.parent_id)" class="unselect material-icons"
        style="vertical-align: middle; margin-left: 2px; font-size: 17px">close</i>
    </div>
  </div>
</div>
</template>
