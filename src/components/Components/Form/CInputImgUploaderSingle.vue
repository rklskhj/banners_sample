<template>
  <div :style="[containerStyle, alignStyle]"
    ...
    <!-- 레이블 -->
    <div v-if="formType.label && label && !onlyBtn" :style="labelStyle" style="width:100%;">{{ label.replace('*', '') }}
        <span v-if="label.indexOf('*')>-1" class="p-highlight">*</span></div>

    <div class="full-width" :style="imgUploaderWrapStyle">
      <div class="flex-center" v-if="!onlyBtn">
        <!-- 이미지 업로더 -->
      ...
      <div id="img-uploader-wrapper" class="flex-center" style="margin-top:12px">
        ...
        <label class="imgUploader-box"
                @click.stop :for="uid" :style="boxStyle">
  ...
  export default {
    ...
    computed: {
      ...
      imgRemoveButtonStyle() {
        if(this.removeBtnCustom) {
          return this.removeBtnCustom;
        }
        else {
          return {
            fontSize: '12px',
    ...
    methods: {
      async onFileChange(e) {
        document.getElementById('img-uploader-wrapper').style.pointerEvents = 'none'
        this.$store.commit('setLoadingSpinner', true);
        let files = e.target.files || e.dataTransfer.files;
        console.log('onfile', files, !files.length)
        if (!files.length) {
          console.log('onfile if')
        ...
        .then(params => {
          let afterAction = (function(file, resizeImg, length, index) {
            ...
            if (length===self.imageArray.length) {
              ...
              document.getElementById('img-uploader-wrapper').style.pointerEvents = ''
              self.imageArray = [];
            }
          }).bind(this);
          this.$store.commit('setLoadingSpinner', false);
