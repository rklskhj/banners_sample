<template>
  <div v-if="product" ref="cardHeight">
    ...
    <div>
      ...
      <!--자체상품 (옵션 없을 경우)-->
      <div v-if="order_product!==undefined && (!product.enable_option || product.enable_option && optionArr1.length === 0)">
        <div class="flex-align size-12"
             :style="[brandColor.sub, brandBgColor.white, pagePadding]"
             style="padding: 14px 0;">
          <div class="size-14 weight-500" :style="brandColor.main" style="width: 125px">수량</div>
          <quantity-input :value.sync="order_product.quantity" :disabled="true"
                          style="width: calc(100% - 125px)"
                          :inputStyle="optionQuantityStyle.input"
                          :min="product.minimum"
                          :max="product.stock_type === 0 && product.stock < product.maximum ? product.stock : product.maximum"
      ...
      <div :style="[pagePadding, {borderTop: brandBorder.main.border}]" style="padding-top: 16px">
        <div class="flex-align full-width" :style="brandColor.main"
             style="margin-bottom:22px">
          <div class="size-14 weight-500" style="width: 125px">상품 총액</div>
          <div class="size-22 weight-700" style="width: calc(100% - 125px)">{{ totalPrice | currency }}</div>
        </div>
      </div>
      ...
      <div v-else-if="product.isOrderable && $store.getters.device !== 'mobile' && product.stock_type === 1"
        ...
        <div class="unselect size-14 bold flex-justify" @click="clickCart"
             ...
             style="margin: 0 8px; width: 50%; height: 48px; border-radius: 8px; align-items: center">장바구니</div>
        <icon-circle-like likeType="product" :style="brandBgColor.sub" @liked="res => toggleLike(res)"></icon-circle-like>
        <!-- <i class="material-icons flex-center" @click="clickCart" :style="[brandBgColor.sub, brandColor.main]"
           style="margin:0 8px;width: 48px; height: 48px; border-radius: 8px;">shopping_cart</i> -->
        <!-- <icon-circle-share @share="$refs.share.share=true" :style="brandBgColor.sub" color="main"></icon-circle-share> -->
  ...
  export default {
    ...
    data () {
      return {
        ...
        soldout_options: [],         // 품절이거나 판매 중지된 상품 옵션 목록
        selectorStocks: [],
        deliveryMethodArr: [],
    ...
    methods: {
      setOptionMax() {
        if (this.order_product.order_product_option.length > 0) {
          // 필수옵션
          let opt1 = this.product.selectors.filter(item=>{ return item.option_type === 0 });
          let opt1_max = 0;
          let opt1_min = 0;
          let ordered = 0;
          if(opt1.length>0) {
            opt1 = opt1[0];
            // 최대 주문 한도
            opt1_max = opt1.maximum;
            opt1_min = opt1.minimum;
            // 스니펫에서 입력한 총 값
            this.order_product.order_product_option.filter(item=>{ return item.option_type === 0 }).forEach(item=>{ ordered += item.quantity });
          }

          // 추가옵션
          let opt2 = this.product.selectors.filter(item=>{ return item.option_type === 1 });
          let opt2_max = opt2.length>0 ? opt2[0].maximum : 1;

          // 각 항목별로 최대값 설정
          this.order_product.order_product_option.forEach(item=>{
            // 필수옵션
            if(item.option_type === 0) {
              item.max = item.quantity < opt1_min ? opt1_min : item.quantity;
              item.min = opt1_min;

              let stock = this.selectorStocks.filter(stock=>{ return stock.id === item.option_id })[0];

              let enable = opt1_max - ordered;
              if(enable<0)
                enable = 0;
              item.max = enable + item.quantity;
              if(stock.enable_stock) {
                if(item.max > stock.stock) {
                  item.max = stock.stock;
                }
              }
            }
            // 추가옵션
              ...
              if(!stock.enable_stock) {
                ...
                item.max = stock.stock < opt2_max ? stock.stock : opt2_max;
              }
            }
          });
        }
              ...
              res.data.stocks.forEach(stock => {
                ...
                res.data.stocks.forEach(stock => {
                  if (stock.option_type === 0) {
                    essentialStocks.push(stock);
                  }
                })
                if (essentialStocks.length !== 0) {
                  let idx = essentialStocks.findIndex(stock => stock.enable_stock === false || (stock.enable_stock && stock.stock !== 0))
                  if (idx === -1) {
                    this.product.stock_type = 0;
                  }
                }
                this.order_product.comm_price = comm_price;
                this.order_product.point = point_price;
              ...
              if(option) {
                ...
                this.order_product.order_product_option.forEach(opt=>{
                  if(opt.option_type === 0)
                    ordered += opt.quantity;
                });
                if(opt1.maximum <= ordered) {
      ...
      pushOption(opt) {
        ...
        if(stock.soldout) {
          ...
          this.order_product.order_product_option.push({option_id: opt.id, quantity: 1, option_type: opt.option_type});
        }
      },
        ...
        if (index !== -1) {
          ...
          if (order_product_option.length !== 0) {
            order_product_option.forEach(option => {
              ...
              if (idx !== -1) {
                if (cart[index].order_product_option[idx].quantity + order_product_option[idx].quantity > order_product_option[idx].max) {
                  this.toast('최대 주문 수량 한도보다 많이 장바구니에 담을 수 없습니다.');
                } else {
                ...
                if (cart[index].quantity + order_product.quantity > this.product.maximum) {
                  this.toast('최대 주문 수량 한도보다 많이 장바구니에 담을 수 없습니다.');
                } else {
      ...
      optionPrice(selectorStocks, option_id) {
        ...
        if (stock) {
          if (stock.option_type === 0) {
            return this.discountedPrice + stock.price;
          } else {
            return stock.price;
          }
        }
      },
      computedOptionMaximum(op) {
        if (op.option_type === 0) {
          if (this.product.stock_type === 0 && this.product.stock < this.product.maximum) {
            return this.product.stock;
          } else {
            return this.product.maximum;
          }
        } else {
          return op.max;
        }
