      ...
      this.$axios.get('public/service/set/filter/'+this.filterId).then(res =>{
        res.data.filter.forEach(filter => {
          ...
          // 레인지 듀얼
            ...
            if(store!==undefined) {
              let min = store.set[0].value === '' ? '' : store.set[0].value;
              let max = store.set[1].value === '' ? '' : store.set[1].value;
      ...
      if(item.type.indexOf('_range')>-1) {
        ...
        if(res[0]!=='' && res[1]!=='') {
          if(item.type.indexOf('number')>-1) {
            if (item.options.unit && item.options.unit !== '') {
              item.selectedValue = `${this.currencyNum(res[0])}${item.options.unit} ~ ${this.currencyNum(res[1])}${item.options.unit}`;
            } else {
              item.selectedValue = `${this.currencyNum(res[0])} ~ ${this.currencyNum(res[1])}`;
            }
          } else {
            if (item.options.unit && item.options.unit !== '') {
              item.selectedValue = `${res[0]}${item.options.unit} ~ ${res[1]}${item.options.unit}`;
            } else {
          ...
          if(item.type.indexOf('number')>-1) {
            if (item.options.unit && item.options.unit !== '') {
              item.selectedValue = `${this.currencyNum(res[0])}${item.options.unit} ~`;
            } else {
              item.selectedValue = `${this.currencyNum(res[0])} ~`;
            }
          } else {
            if (item.options.unit && item.options.unit !== '') {
              item.selectedValue = `${res[0]}${item.options.unit} ~`;
            } else {
              item.selectedValue = `${res[0]} ~`;
            }
          }
        } else if (res[0] === '' && res[1] !== '') {
          if(item.type.indexOf('number')>-1) {
            if (item.options.unit && item.options.unit !== '') {
              item.selectedValue = `~ ${this.currencyNum(res[1])}${item.options.unit}`;
            } else {
              item.selectedValue = `~ ${this.currencyNum(res[1])}`;
            }
          } else {
            if (item.options.unit && item.options.unit !== '') {
              item.selectedValue = `~ ${res[1]}${item.options.unit}`;
            } else {
              item.selectedValue = `~ ${res[1]}`;
            }
          }
        } else {
          item.selectedValue = ''
