...
<div class="p-border-bottom-sub" style="padding:16px 0">
    ...
    <div class="flex-align full-width" @click="clickCard">
      ...
      <div style="padding-left:12px;flex:1">
        ...
        <div class="size-16 weight-500 p-main">{{ card.product }}</div>
        <div class="size-12 p-sub2">{{ card.product_info.simple_desc }}</div>
      </div>
    </div>
    ...
    <div class="flex-between" style="margin:12px 0 4px 0">
      <div class="flex-align">
        ...
        <div class="size-12 p-sub2" style="margin-left:12px">
          <i class="material-icons-outlined" v-if="card.is_secret" style="margin-right:4px;vertical-align:sub;font-size:16px">lock</i>
          {{ dateFormat(card.created_time, 'ago') }}</div>
      </div>
      <div class="flex-align">
        <div class="btn size-14 radius-6 unselect p-border-brand p-primary"
          @click="clickEdit">수정</div>
        <div class="btn size-14 radius-6 unselect p-border-sub p-sub2"
          @click="clickDelete">삭제</div>
      </div>
    </div>
    
    <!-- 작성 내용 -->
    <div class="size-14 weight-500 p-main"><span class="weight-700 p-primary" style="margin-right:4px">Q</span>{{ card.title }}</div>
    <div class="size-14 p-sub" style="margin-top:2px;line-height:20px">{{ card.question_content }}</div>
    <!-- 답변 -->
    <template v-if="card.answerer">
      <div class="size-14 weight-500 p-main" style="margin-top:24px"><span class="weight-700 p-highlight" style="margin-right:4px">A</span>답변내용</div>
      <div class="size-14 p-sub" style="margin-top:2px;line-height:20px">{{ card.answer_content }}</div>
