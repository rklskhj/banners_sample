...
<div @click="clickCard" class="unselect full-width">
    <div class="card-partner full-width flex-align">
      <div class="position-relative" style="width:96px" ref="card">
      ...
      <div style="flex:1;padding-left:20px">
        ...
        <template v-if="cardProps.visible.simple_desc && card.simple_desc !== ''">
          <div style="width:100%" class="size-13 ellipsis p-sub2">{{ card.simple_desc }}</div>
        </template>
        ...
        <div v-if="cardProps.visible.address && card.address !== ''" style="width:100%"
      ...
      <div v-if="cardProps.visible.like">
          <icon-btn-like likeType="partner" :id="card.id"
            activeColor="highlight" color="e3e3e3"
...
export default {
  ...
  methods: {
    clickCard() {
      if (this.$route.path === '/map_search') return;
      else {
