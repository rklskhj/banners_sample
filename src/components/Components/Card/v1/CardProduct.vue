    ...
    <div class="card-product full-width" ref="card">
      <e-image :isBg="true" :isLazy="false" v-if="width>0"
        ...
        <div v-if="soldout" slot="inner"
          class="position-absolute prod-unavailable size-13 p-white">{{ card.soldout ? '품절': '예약마감' }}</div>
      </e-image>
      ...
      <div :style="brandBgColor.white" style="padding-top:8px">
        ...
        <template v-if="cardProps.visible.product.simple_desc && card.simple_desc !== ''">
        ...
        <template v-if="card.price.price_type && card.price.price_type===1">
          <div v-if="cardProps.visible.product.price.price"
            ...
            class="size-15 weight-600 margin-bottom-4 p-main">{{ card.price.price_text }}</div>
        </template>
        <template v-else>
          ...
          <div class="flex-align margin-bottom-2" v-if="card.price"
            ...
            <template v-if="isDiscount && cardProps.visible.product.price.discount_price">
              <div class="size-16 weight-700 p-highlight">
                <span v-if="card.price.discount_type===0 && card.price.discount_rate > 0" style="margin-right:8px">{{ card.price.discount_rate }}%</span>
                <span v-else-if="card.price.discount_type===1 && card.price.discount_price > 0" style="margin-right:8px">{{ card.price.discount_price | currency }}</span>
              </div>
            </template>
            <div v-if="cardProps.visible.product.price.price"
        ...
        <div class="flex-align weight-700 margin-bottom-6 p-main">
          <div v-if="cardProps.visible.product.rate" style="margin-right:6px"
            class="flex-align">
        ...
        <div v-if="cardProps.visible.product.address && card.address !== ''" style="width:100%;white-space:pre-line"
            class="size-12 ellipsis p-sub2">{{ card.address }}</div>
