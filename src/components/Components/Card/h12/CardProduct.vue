...
<!--<a :href="`product_detail?id=${card.id}`" target="_blank">-->
    ...
    <div class="flex-align" style="align-items: flex-start">
      <div class="position-relative" style="width:33%" ref="card">
        <e-image :imageRates="cardProps.imageRates" :isBg="true" :img="card.img"
          ...
          <div v-if="soldout" slot="inner"
                class="position-absolute prod-unavailable size-12 p-white">{{ card.soldout ? '품절': '예약마감' }}</div>
        </e-image>
      </div>
      ...
      <div style="flex:1;padding-left:16px">
        ...
        <template v-if="card.price.price_type && card.price.price_type===1">
          <div v-if="cardProps.visible.product.price.price"
            class="size-15 weight-600 margin-bottom-4 p-main"
              :class="{'soldout': soldout}">{{ card.price.price_text }}</div>
        </template>
        <template v-else>
          <div class="size-12 weight-400 p-sub2"
            :class="{'soldout': soldout}"
              v-if="isDiscount && cardProps.visible.product.price.org_price && card.price.discount_type === 0 && card.price.discount_type > 0"
          ...
          <div class="flex-align margin-bottom-4" v-if="card.price"
            ...
            <template v-if="isDiscount && cardProps.visible.product.price.discount_price">
              <div v-if="card.price.discount_type===0 && card.price.discount_type > 0"
