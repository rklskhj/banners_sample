    ...
    <div :style="pcMaxWidth">
      <div class="list-wrap-grid">
        <div class="flex-align">
          ...
          <div v-else>
            <div class="full-width margin-bottom-16" :style="menuTtitle">
              {{ title }}
            </div>
        ...
        <div class="overflow-hidden">
          ...
          >
            <div class="grap-bing flex-align full-width">
              <div
                v-for="(category, idx) in categories"
                :key="'category-' + idx"
                class="unselect list-item p-main size-16"
                :class="{ bold: category.id === selectedCategory.id }"
...
export default {
  ...
  methods: {
    ...
    onDrag(e) {
      // 기종별 감도차이 떄문에 0이 아닌 1로 약간의 여유있게 줌
    ...
    getCategoryProductList(category) {
      ...
      category.product_list.forEach((item) => {
        if (
          item.order_conversion_type !== 10 &&
          item.order_conversion_type !==
            11(item.stock_type !== 0 || item.stock !== 0)
        ) {
          arr.push(item);
        } else if (
          item.order_conversion_type === 10 &&
