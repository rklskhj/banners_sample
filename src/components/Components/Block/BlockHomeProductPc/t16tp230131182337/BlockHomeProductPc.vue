    ...
    <div class="list-wrap-grid">
      <div class="flex-align">
        <div v-if="type==='exhibition'">
          <div class="full-width margin-bottom-16"
            :style="menuTitle">{{ exhibition.title }}</div>
          <div class="full-width" :style="menuSubtitle"
            style="margin-bottom:56px">{{ exhibition.subtitle }}</div>
          <div v-if="list && list.length > 0"
            class="size-16 unselect btn-more text-center radius-8 p-border-primary p-primary"
            @click="goExhibition">기획전 더보기</div>
        </div>
      ...
      <div class="overflow-hidden">
        <div v-if="useCategory && categories && categories.length > 0"
          style="margin-top: 24px; padding-top: 10px; margin-bottom: 32px; height: 60px"
        ...
        <div v-else-if="list && list.length === 0"
          ...
          style="align-items: center; height: 20vh">{{ noListText }}</div>
      </div>
    </div>
  </div>
</div>
