<template>
<div :style="[pagePadding,containerBg]" style="min-height:200px;padding:25px 0">
  <div style="padding-bottom:16px">
    <div v-if="type==='exhibition'">
      <div class="full-width flex-between">
        <div class="unselect" :style="menuTitle">
  ...
  <div v-else-if="list && list.length === 0"
    ...
    style="align-items: center" :style="{height: '20vh'}">{{ noListText }}</div>
  
  <div v-if="useCategory && list && list.length === 0" style="margin-top: 20px">
...
export default {
  ...
  methods: {
    ...
    getProducts() {
      ...
      let url = `user/${this.user.user_id}/mapping/product${fields}&page_num=1&page_length=6`;
    ...
    cardStyle(idx) {
      ...
      if(idx%2===0) {
        deco.paddingRight = '8px'
      }
      else if(idx%2===1) {
        deco.paddingLeft = '8px'
      }
