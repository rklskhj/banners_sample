    ...
    <div v-if="optionState" class="full-width position-fixed" 
      ...
      <div class="full-width position-absolute option-content p-bg-white"
        ...
        <div v-if="product" ref="cardHeight"
          style="min-height: 40vh; max-height: 40vh; overflow-y: auto; padding: 16px 16px 22px">
          <!-- 주문 버튼 -->
          <div>
            <!-- 옵션 사용 -->
            <div v-if="product.enable_option">
              ...
              <div v-if="optionArr2.length>0" style="margin-top: 16px">
            ...
            <div v-if="order_product!==undefined && product.enable_option && order_product.order_product_option.length > 0"
              ...
              <div 
                ...
                :style="selectorStockStyle(idx)" style="padding:16px">
                <div class="flex-between full-width" style="gap: 0 14px">
                  <div style="width: calc(100% - 34px)">
                    <div class="size-13 p-sub flex" style="gap: 0 4px">
                      <div v-if="optionType(selectorStocks, op.option_id)===0"
                        class="weight-700">옵션</div>
                      <div v-if="optionType(selectorStocks, op.option_id)===1"
                        class="weight-700">추가옵션</div>
                      <div class="ellipsis flex-1">{{ optionName(selectorStocks, op.option_id) }}</div>
                    </div>
                  </div>
                  <div class="unselect p-sub3">
                    <i v-if="op.option_id" class="material-icons"
            ...
            <div v-if="order_product!==undefined && !product.enable_option">
              <div class="flex-between size-12 radius-8 p-sub p-bg-sub" :style="pagePaddingAll">
        ...
        <div class="p-border-top-sub"
          ...
          <div class="flex-between full-width p-main">
            <div class="size-14 weight-500 p-sub">총 금액 ({{ totalCount }})</div>
            <div class="size-18 weight-700">{{ totalPrice | currencyNum }}원</div>
          </div>
          <div class="flex-align" style="margin-top: 8px; gap: 0 10px">
            <div @click="clickCart" class="unselect col-6 size-14 flex-center radius-8 p-border-primary p-primary"
              style="height: 48px">장바구니 담기</div>
            <div @click="clickOrder"
              class="button unselect col-6 flex-center size-14 radius-8 p-white p-bg-primary"
              style="height: 48px">구매하기</div>
          </div>
        </div>
...
export default {
  name: 'BlockOptionChooseActionSheet',
  components: {
  ...
  computed: {
    totalCount() {
      if (this.product.enable_option) {
        return this.currencyNum(this.order_product.order_product_option.reduce((acc, cur) => acc + cur.quantity, 0))
      } else {
        return this.currencyNum(this.order_product.quantity)
      }
    },
    optionModalStyle() {
      let deco = {
