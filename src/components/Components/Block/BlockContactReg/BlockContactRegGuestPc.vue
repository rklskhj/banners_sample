...
<div style="padding:32px 0 100px;min-height:100vh" :style="pcMaxWidth" class="position-relative">
    ...
    <div>
      ...
      <div class="flex-align">
        ...
        <div class="size-14 p-sub" style="margin-left: 6px">
          회원으로 이용하실 분은 로그인 해 주시기 바랍니다.</div>
      </div>
    </div>
    <div @click="$router.push('/signin')" class="flex-justify unselect radius-8 p-border-main p-main"
      style="width: 76px; height: 44px; align-items: center">로그인</div>
  </div>
  <div v-else class="size-24 weight-500 p-main">1:1 문의 작성</div>
  <div class="p-border-top-main p-border-bottom-main"
  ...
  <div class="flex-align" style="margin-top:80px; justify-content: center">
    <div :style="buttonCustomCancel" style="margin-right:16px" class="unselect pc-style" @click="clickBack">취소</div>
    <div :style="buttonCustom" class="unselect pc-style" @click="register">문의 작성</div>         
  </div>
</div>
</template>
<script>
        ...
        .then(res => {
          ...
          .then(res2 => {
            ...
            res2.data.forEach(item => {
              console.log('getform',this.isLogin, item)
            ...
            result.push(
            {
              ...
              value: false,
              containerCustom: {marginTop: '16px'} ,
              boxCustom: {
                width: '32px',
                height: '32px',
                borderRadius: '8px',
