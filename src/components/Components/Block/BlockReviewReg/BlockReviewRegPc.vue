...
<div :style="pcMaxWidth" style="padding-top:40px">
  <div class="size-24 weight-700" :style="brandColor.main" style="margin-bottom: 24px">상품 후기 등록</div>
  
  <!-- 후기 작성 -->
  <div style="padding:16px 0 12px 0" :style="`border-top:${brandBorder.sub.border}`">
  ...
  <div class="flex-center" style="margin-top:100px">
    <button :style="[brandBorder.main,brandColor.sub2,brandBgColor.white]" style="margin-right: 8px" @click="$router.back()">취소</button>
    <button :style="[brandBgColor.primary, brandBorder.primary, brandColor.white]" style="margin-left: 8px" @click="save">등록하기</button>
  </div>
</div>
</template>
<script>
...
export default {
  ...
  created() {
      ...
      this.labelStyle.color = this.brandColor.sub.color;
  ...
  methods: {
    ...
    appendParamData(params) {
        ...
        if (c_type===3) {
          // delivery
          params.order_product_id = this.order_id;
        } else if (c_type===4) {
          // meet
          params.meet_order_id = this.order_id;
        } else if (c_type===8) {
          // commerce
          params.commerce_order_product_id = this.order_id;
        } else if (c_type===10) {
          // reservation
          params.reservation_order_id = this.order_id;
        } else if (c_type===11) {
          // booking
          params.booking_order_id = this.order_id;
        } else if (c_type===12) {
          // mart
          params.mart_order_product_id = this.order_id;
        }
      }
  },
  computed: {
