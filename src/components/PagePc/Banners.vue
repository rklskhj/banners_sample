...
<script>
import BannersPc from "@/components/Pages/BannersPc.vue";
...
export default {
        ...
        if (!this.isLogin) {
          ...
          if (partner_grade > 0) {
            ...
            // 파트너일땐, 등급 안맞는 페이지로 이동
              ...
              this.$router.replace(
                "/wrong_partner_grade?order=" + partner_grade
              );
            }
          } else if (grade > 0) {
  ...
  methods: {
    ...
    getOrderConversion(board) {
      let oc = board.filter(
        (i) =>
          this.order_conversion_type &&
          this.order_conversion_type === i.order_conversion_type
      );
    ...
    getProductCategories() {
      return new Promise((resolve) => {
        if (this.$route.path === "/product_detail") {
          this.$axios
            .get(
              `/user/0/mapping/product/${this.$route.query.id}?fields=category1,category2,category3,order_conversion_type`
            )
            .then((cat_res) => {
    ...
    dashToPascal(value) {
      let camel = value.replace(/([-_][a-z])/gi, ($1) => {
        return $1.toUpperCase().replace("-", "").replace("_", "");
      });
      return camel[0].toUpperCase() + camel.substring(1);
    },
    getBoardsGuest() {
      let board = this.boards.filter((item) => {
    ...
    getBoardsUserGrade(grade) {
      let board = this.boards.filter((item) => {
        return (
          item.activated && item.login_required === true && item.grade === grade
        );
      });
    ...
    getCategories(boards, guest, grade) {
      if (
        ["/product", "/search", "/product_detail", "/filter"].indexOf(
          this.$route.path
        ) === -1 ||
        boards.length === 0
      ) {
      ...
      let category_id = this.$route.query.gnb
        ? this.$route.query.gnb
      ...
      if (this.$route.query.category3) {
        cat_board = boards.filter((item) => {
          return item.category3 === Number(this.$route.query.category3);
        });
      ...
      if (cat_board.length === 0 && this.$route.query.category2) {
        cat_board = boards.filter((item) => {
          return item.category2 === Number(this.$route.query.category2);
        });
        if (cat_board.length > 0) {
      ...
      if (cat_board.length === 0 && this.$route.query.category1) {
        cat_board = boards.filter((item) => {
          return item.category1 === Number(this.$route.query.category1);
        });
        if (cat_board.length > 0) {
      ...
      if (category_id > 0) {
        if (this.category && this.category.order_conversion_type) {
          cat_board = boards.filter((item) => {
            return (
              item.order_conversion_type === this.category.order_conversion_type
            );
          });
          if (cat_board.length > 0) {
            return cat_board;
        ...
        if (category) {
          ...
          cat_board = boards.filter((item) => {
            ...
            return (
              category.packages.filter((item2) => {
                return (
                  item2 === order_conversion_type[item.order_conversion_type]
                );
              }).length > 0
            );
          });
      ...
      if (category_id === undefined) {
        ...
        let board_all = boards.filter((item) => {
      ...
      cat_board = boards.filter((item) => {
        return (
          item.category1 === category_id ||
          item.category2 === category_id ||
          item.category3 === category_id
        );
      });
      ...
      if (cat_board.length === 0) {
        cat_board = boards.filter((item) => {
          return (
            item.category1 === this.category.category1 &&
            item.category2 === this.category.category2
          );
        });

        if (cat_board.length === 0) {
          cat_board = boards.filter((item) => {
            return item.category1 === this.category.category1;
          });
        }
